...
def u3pblock(in_ch, out_ch, num_block=2, kernel_size=3, padding=1, down_sample=False):
    m = []
    if down_sample:
        # m.append(nn.MaxPool3d(kernel_size=2))
        m.append(nn.Conv3d(in_ch, out_ch, kernel_size, stride=2, padding=padding, bias=False))
        in_ch = out_ch
    for _ in range(num_block):
        m += [nn.Conv3d(in_ch, out_ch, kernel_size, bias=False, padding=padding),
...
def en2dec_layer(in_ch, out_ch, scale):
    if scale == 1:
        ...
        m = [nn.Conv3d(in_ch, in_ch, kernel_size=scale, stride=scale, padding=0)]
        # m = [nn.MaxPool2d(scale, scale, ceil_mode=True)]
    m.append(u3pblock(in_ch, out_ch, num_block=1))
    return nn.Sequential(*m)

def dec2dec_layer(in_ch, out_ch, scale, fast_up=True):
    up = [nn.Upsample(scale_factor=scale, mode='trilinear', align_corners=True) if scale != 1 else nn.Identity()]
    ...
    def __init__(self, en_channels = [64, 128, 256, 512, 1024], skip_ch=64, dropout=0.3, fast_up=True):
        ...
        for ii in range(num_en_ch):
            ...
            self.decoders[f'decoder{ii+1}'] = FullScaleSkipConnect(
                                                en_channels[ii:], 
                                                en_scales=[pow(2, i) for i in range(num_en_ch - ii)],
                                                num_dec=ii, 
                                                skip_ch=skip_ch, 
...
class UNet3Plus(nn.Module):
    ...
    def __init__(self, 
        ...
        if transpose_final:
            self.head = nn.Sequential(
                nn.ConvTranspose3d(decoder_ch, num_classes, kernel_size=4, stride = 2, padding=1, bias=False),
            )
        else:
            self.head = nn.Conv3d(decoder_ch, num_classes, 3, padding=1)
    ...
    def forward(self, x): 
        ...
        pred = self.resize(self.head(de_out[-1]), h, w, d)
        
        return pred
    
    def resize(self, x, h, w, d) -> torch.Tensor:
        _, _, xh, xw, xd = x.shape
        if xh != h or xw != w or xd != d:
            x = F.interpolate(x, size=(h, w, d), mode='trilinear', align_corners=True)
        return x


if __name__ == '__main__':
    # input_ = torch.randn((1, 1, 96,96,128))
    model = UNet3Plus(num_classes=1, in_channels=1, channels=[16, 32, 64])
    # out = model(input_)
    # print(out.shape)
